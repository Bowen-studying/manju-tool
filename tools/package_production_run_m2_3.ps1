...
$archivePath = Join-Path ([System.IO.Path]::GetTempPath()) ("manju-m23-" + [guid]::NewGuid().ToString("N") + ".zip")
try {
    ...
    # Archive supplies the immutable base.  Overlay every executable source and
    # test from the current worktree, not a hand-maintained dependency list:
    # M2.3 is tested from a dirty tree and a single omitted module can otherwise
    # silently regress to HEAD (as happened with storyboard_supervisor.py).
    $overlay = @(
        "README.md", "pyproject.toml", "requirements.txt", "uv.lock",
        "docs\PRODUCTION_RUN_M2_3_TESTING.md", "tools\package_production_run_m2_3.ps1"
    )
